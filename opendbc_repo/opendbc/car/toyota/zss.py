...
class ZSS:
  ...
  def __init__(self, flags: int):
    ...
    self._zss_value = 0.
    self._disable_print_allow = True
  ...
  def get_steering_angle_deg(self, main_on, cruise_active, stock_steering_angle_deg):
    ...
    # alka just activated
    if not self._offset_compute_required and ((alka_active and not self._alka_active_prev) or (cruise_active and not self._cruise_active_prev)):
      self._threshold_count = 0
      self._offset_compute_required = True
      self._disable_print_allow = True
    ...
    if self._threshold_count >= THRESHOLD_COUNT:
      if self._disable_print_allow:
#        print("ZSS: Too many large diff, fallback to stock.")
        self._disable_print_allow = False
      return stock_steering_angle_deg
...
#      print("ZSS: Diff too large, fallback to stock. ")
...
#      print(f"ZSS: offset computed: {self._offset_compute_required}")
